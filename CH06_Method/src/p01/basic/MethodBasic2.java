...
/*
	...
	3. method 사용 효과
		- 전체 소스코드 양이 대폭 줄일 수 있고, 프로그램 내에서 중복되는 코드 덩어리를 제거 가능 
		- 중복되는 코드 덩어리를 수정할 경우, 메소드 내부에서만 수정하면 되어서 전체 코딩 속도 향상 가능.
	4. 회원가입 여부 check, 쇼핑몰 상품 가격이 1000원 이상인 경우 상품 가격 입력시 상품가격 체크 등.
...
public class MethodBasic2 {
	...
	public static void main(String[] args) {
		...
		// 1, 10 => actual parameter list (arguments: 인수)
		
		System.out.println("1과 10사이의 합 : " + sum(1,10)); 		// sum() 메소드 호출.
		/*
			sum(1,10)을 호출하면 (기계어로 처리되는 명령어 리스트)
			1. 등록된 method list에서 sum이라는 이름을 가진 method signature를 이용하여 찾음.
			2. 찾은 sum method을 호출하고, method의 formal parameter인 startValue 변수의 메모리방에 1의 값을 넣고
